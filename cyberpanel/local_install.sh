...
for i in {1..50} ;
  do
  if [[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "9" ]] || [[ "$Server_OS_Version" = "24" ]]; then
   # wget -O /usr/local/requirments.txt "${Git_Content_URL}/${Branch_Name}/requirments.txt"
   cp requirements.txt /usr/local/requirments.txt
  else
   # wget -O /usr/local/requirments.txt "${Git_Content_URL}/${Branch_Name}/requirments-old.txt"
   cp requirments-old.txt /usr/local/requirments.txt
  fi
...
if [[ "$Server_OS" = "CentOS" ]] || [[ "$Server_OS" = "openEuler" ]] ; then
  ...
  if [[ "$Server_OS_Version" = "22" ]] || [[ "$Server_OS_Version" = "24" ]] ; then
    DEBIAN_FRONTEND=noninteractive apt install -y dnsutils net-tools htop telnet libcurl4-gnutls-dev libgnutls28-dev libgcrypt20-dev libattr1 libattr1-dev liblzma-dev libgpgme-dev libcurl4-gnutls-dev libssl-dev nghttp2 libnghttp2-dev idn2 libidn2-dev libidn2-0-dev librtmp-dev libpsl-dev nettle-dev libgnutls28-dev libldap2-dev libgssapi-krb5-2 libk5crypto3 libkrb5-dev libcomerr2 libldap2-dev git socat vim unzip zip libmariadb-dev-compat libmariadb-dev python3-virtualenv
     Check_Return
  else
...
if [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "24" ]]; then
    echo "Skipping pip install virtualenv for Ubuntu 24..."
elif [[ "$Server_OS" = "Ubuntu" ]] && [[ "$Server_OS_Version" = "22" ]]; then
   echo "Skipping pip install virtualenv for Ubuntu 22..."
else
...
Retry_Command "pip install --no-build-isolation --default-timeout=3600 -r /usr/local/requirments.txt"
...
rm -rf cyberpanel
echo -e "\nUsing local CyberPanel source code...\n"
Debug_Log2 "Using local CyberPanel code..,4"

# Use rsync if available, else cp
if command -v rsync >/dev/null 2>&1; then
    mkdir -p /usr/local/cyberpanel
    rsync -av --exclude '.git' --exclude 'local_install.sh' . /usr/local/cyberpanel
else
    mkdir -p /usr/local/cyberpanel
